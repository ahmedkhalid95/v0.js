(function() {
    ...
    function Xa(a, b) {
        for (var c, d = a.ca, e = k; !d.f();) {
            c = a.Oa(b, d.key);
            if (0 === c) {
                if (d.right.f()) return e ? e.key : k;
                for (d = d.right; !d.left.f();) d = d.left;
                return d.key
            }
            0 > c ? d = d.right : 0 < c && (e = d, d = d.left)
        }
    ...
    function Ya(a, b) {
        this.md = b;
        for (this.Tb = []; !a.f();) this.Tb.push(a), a = a.right
    }
    ...
    function Za(a) {
        ...
        for (b = b.left; !b.f();) a.Tb.push(b), b = b.right;
        return c
    }
    ...
    r.copy = function(a, b, c, d, e) {
        return new $a(a != k ? a : this.key, b != k ? b : this.value, c != k ? c : this.color, d != k ? d : this.right, e != k ? e : this.left)
    };
    r.count = function() {
        return this.right.count() + 1 + this.left.count()
    };
    r.f = o(l);
    r.Ca = function(a) {
        return this.right.Ca(a) || a(this.key, this.value) || this.left.Ca(a)
    };
    r.Ma = function(a) {
        return this.left.Ma(a) || a(this.key, this.value) || this.right.Ma(a)
    };

    function ab(a) {
        return a.right.f() ? a : ab(a.right)
    }
    r.tb = function() {
        return ab(this).key
    };
    r.Va = function() {
        return this.left.f() ? this.key : this.left.Va()
    };
    r.oa = function(a, b, c) {
        var d, e;
        e = this;
        d = c(a, e.key);
        e = 0 > d ? e.copy(k, k, k, e.right.oa(a, b, c), k) : 0 === d ? e.copy(k, b, k, k, k) : e.copy(k, k, k, k, e.left.oa(a, b, c));
        return db(e)
    };

    function eb(a) {
        if (a.right.f()) return Wa;
        !a.right.O() && !a.right.right.O() && (a = fb(a));
        a = a.copy(k, k, k, eb(a.right), k);
        return db(a)
    }
    r.remove = function(a, b) {
        var c, d;
        c = this;
        if (0 > b(a, c.key)) !c.right.f() && (!c.right.O() && !c.right.right.O()) && (c = fb(c)), c = c.copy(k, k, k, c.right.remove(a, b), k);
        else {
            c.right.O() && (c = gb(c));
            !c.left.f() && (!c.left.O() && !c.left.right.O()) && (c = hb(c), c.right.right.O() && (c = gb(c), c = hb(c)));
            if (0 === b(a, c.key)) {
                if (c.left.f()) return Wa;
                d = ab(c.left);
                c = c.copy(d.key, d.value, k, k, eb(c.left))
            }
            c = c.copy(k, k, k, k, c.left.remove(a, b))
        }
        return db(c)
    };
    r.O = ba("color");
    ...
    function db(a) {
        a.left.O() && !a.right.O() && (a = ib(a));
        a.right.O() && a.right.right.O() && (a = gb(a));
        a.right.O() && a.left.O() && (a = hb(a));
        return a
    }

    function fb(a) {
        a = hb(a);
        a.left.right.O() && (a = a.copy(k, k, k, k, gb(a.left)), a = ib(a), a = hb(a));
        return a
    }

    function ib(a) {
        var b;
        b = a.copy(k, k, j, k, a.left.right);
        return a.left.copy(k, k, a.color, b, k)
    }

    function gb(a) {
        var b;
        b = a.copy(k, k, j, a.right.left, k);
        return a.right.copy(k, k, a.color, k, b)
    }

    function hb(a) {
        var b, c;
        b = a.right.copy(k, k, !a.right.color, k, k);
        c = a.left.copy(k, k, !a.left.color, k, k);
